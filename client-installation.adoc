...
The exact steps to add a new directory to the executable search path can
vary by the version of Windows being run.
...
Tab__, and then finally click on __Environment Variables__. Once the new
dialog opens, select the __Path__ variable and add ";C:\OpenShift" at the
end.

Instead of updating the excutable search path, you could always just copy
